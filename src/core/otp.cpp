...
uint64_t Totp::_get_counter(time_t* time) {
  if (time) {
    Totp::_remaining_time = Totp::_time_step - (*time % Totp::_time_step);
    return *time / Totp::_time_step;
  }

  Totp::_remaining_time = Totp::_time_step - (Totp::_cur_time % Totp::_time_step);
